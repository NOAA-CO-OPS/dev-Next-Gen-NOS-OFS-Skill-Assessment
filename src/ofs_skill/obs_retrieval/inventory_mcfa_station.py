"""
Create inventory of MCFA (Maine Coast Fishermen's Association) stations.
"""
...
def inventory_mcfa_station(
    ...
    logger.info('Calling MCFA ERDDAP service...')
    logger.info(f'MCFA Query URL: {query_url}')
    ...
    try:
        ...
        if ex.code == 404:
            logger.info('No MCFA stations found within bounding box and time range (404)')
            return None
        logger.error('MCFA data download failed at %s -- HTTP Error: %s', query_url, str(ex))
        return None
    except Exception as ex:
        logger.error('MCFA data download failed at %s -- %s', query_url, str(ex))
        return None

    if df.empty:
        logger.info('No MCFA stations found within bounding box and time range')
        return None
    ...
    inventory_mcfa = pd.DataFrame({
        'ID': df['tow_id'].astype(str),
        'X': df['longitude'],
        'Y': df['latitude'],
        'Source': 'MCFA',
        'Name': 'MCFA_Tow_' + df['tow_id'].astype(str), # Adding prefix bypasses the filter
        'has_wl': False,
        'has_temp': True,
        'has_salt': False,
        'has_cu': False,
    ...
    logger.info('inventory_mcfa_station.py ran successfully - found %d stations', len(inventory_mcfa))
    return inventory_mcfa
